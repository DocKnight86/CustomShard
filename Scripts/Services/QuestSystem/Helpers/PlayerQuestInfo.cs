    ...
    public class PlayerQuestInfoGump : BaseGump
    {
        ...
        public static void CheckQuests(CommandEventArgs e)
        {
            if (e.Mobile is PlayerMobile pm)
            {
                pm.BeginTarget(-1, false, TargetFlags.None, (m, targeted) =>
                {
                    ...
                    {
                        List<BaseQuest> quests = subject.Quests;
                        Dictionary<QuestChain, BaseChain> chains = subject.Chains;
                        List<QuestRestartInfo> done = subject.DoneQuests;
        ...
        private void BuildRestart()
        {
            ...
            if (done != null)
            {
                ...
                {
                    QuestRestartInfo info = done[i];

                    if (info != null)
                    {
                        BaseQuest quest = QuestHelper.Construct(info.QuestType);
            ...
            switch (info.ButtonID)
            {
                ...
                default:
                    if (info.ButtonID >= 1000)
                    {
                        ...
                        {
                            QuestRestartInfo restart = done[id];
