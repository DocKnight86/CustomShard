...
namespace Server.Mobiles
{
    ...
    public class AbyssalInfernal : BaseChampion
    {
            ...
            if (map != null)
            {
                ...
                if (toCondemn != null && !m_Table.ContainsKey(toCondemn))
                {
                    ...
                    Timer.DelayCall(TimeSpan.FromSeconds(seconds), () =>
                    {
                        ...
                        foreach (KeyValuePair<Mobile, Point3D> pair in m_Table)
                        {
                            list.Add(pair);
                        }
        ...
        public int TotalSummons()
        {
            ...
            for (int index = 0; index < SummonedHelpers.Count; index++)
            {
                BaseCreature bc = SummonedHelpers[index];
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            if (SummonedHelpers != null)
            {
                for (int index = 0; index < SummonedHelpers.Count; index++)
                {
                    BaseCreature m = SummonedHelpers[index];
                    writer.Write(m);
                }
            }
        }
